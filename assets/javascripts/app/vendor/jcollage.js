// taken from http://radikalfx.com/2009/10/16/canvas-collage/
// hacked to remove ui components which are not needed
function Collage($canvas) {
  ...
  this.scaleLayer = function(layer, width, height){
    var square = layer.getSquare();
    var scaleToWidth = Math.sqrt(Math.pow(square.d.x - width, 2) + Math.pow(square.d.y - height, 2));
    var scaleToHeight = Math.sqrt(Math.pow(square.b.x - width, 2) + Math.pow(square.b.y - height, 2));

    layer.scale(scaleToWidth, scaleToHeight);
    drawMarker(layer);
  };
